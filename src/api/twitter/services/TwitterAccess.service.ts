...
export class TwitterAccessService {
  public async oAuthRequestToken(data: { userId: string; }): Promise<UserToken> {
    try {
      ...
      // log for debugging and run support purposes
      logger.error(`{}TwitterService::#oAuthRequestToken::error executing::error=${utils.anyy.stringify(error)}`);
  ...
  public async oAuthAccessToken(
    ...
    try {
      ...
      const error = new APIError(err);
      // log for debugging and run support purposes
      logger.error(`{}TwitterService::#oAuthAccessToken::error executing::error=${utils.anyy.stringify(error)}`);
      // throw error explicitly
      throw error;
    }
  }
