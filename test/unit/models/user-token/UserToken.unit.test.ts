...
// tests
describe('models/user-token/UserToken unit tests', () => {
  before(async () => {
    try {
      // setup
      // user tokens
    ...
    describe('#validate', () => {
      ...
      it('- should validate (synchronously) a UserToken class instance and return false when the data does not match the defined schema', () => {
        try {
          ...
          // set expectations
          const EXPECTED_VALIDATION_ERROR: any = `Type must be one of the following values: ${testUtils.enumerations.enumerate(UserTokenTypeEnum).join(', ')}`;
